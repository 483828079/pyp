...
<head>
    ...
    <script>
        /*初始化sku列表，默认sku排在前面*/
        /*直接初始化所有sku，当规格选项卡选中的时候只用在页面上修改
        * sku信息，不用再次生成页面，或者查询数据库。
        * */
        var skuList = [
...
<#include "head.ftl"/>
<#--图片列表，将原来的字符串格式json转换为对象-->
<#if goodsDesc.itemImages ??>
...
购热销商品</em>
					...
					<div class="clearfix choose">
						<div id="specification" class="summary-wrap clearfix">
							<#if specificationList ?? && (specificationList ? size > 0)>
								...
								<#--加了效果的-->
										<#if specification.attributeValue ?? && (specification.attributeValue ? size > 0)>
											<#list specification.attributeValue as item>
												<dd ng-click="selectSpecification('${specification.attributeName}','${item}')"><a href="javascript:;" class="{{isSelected('${specification.attributeName}','${item}') ? 'selected' : ''}}">${item}金色<span title="点击取消选择">&nbsp;</a></dd>
											</#list>
    									</#if>
						...
						<div class="tab-content tab-wraped">
							<div id="one" class="tab-pane active">
								<ul class="goods-intro unstyled">
									<#if customAttributeList?? && (customAttributeList ? size > 0)>
								...
								<div class="intro-detail">
									<#if goodsDesc.introduction ??>
										${goodsDesc.introduction}
									</#if>
								</div>
							</div>
							<div id="two" class="tab-pane">
								<#if goodsDesc.packageList ??>
									<p>${goodsDesc.packageList}</p>
								</#if>
							</div>
							<div id="three" class="tab-pane">
								<#if goodsDesc.saleService ??>
